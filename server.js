...
// Rutas de páginas
app.get('/',       (req, res) => res.sendFile(path.join(__dirname, 'public', 'index.html')));   // landing
app.get('/login',  (req, res) => res.sendFile(path.join(__dirname, 'public', 'login.html')));   // login
app.get('/app',    (req, res) => res.sendFile(path.join(__dirname, 'public', 'app.html')));     // la app
  ...
  try {
    const pref = await mpPreference.create({
      body: {
        ...
        back_urls: {
          success: `${baseUrl}/?pago=ok&plan=${plan}`,
...
// ════════════════════════════════════════════════════════════════
app.post('/api/remove-bg', requireAuth, upload.single('image'), requireCredits('remove-bg'), async (req, res) => {
  if (!req.file) return res.status(400).json({ success: false, error: 'No se recibió ninguna imagen.' });

  console.log(`[remove-bg] ${req.user.email} · ${req.file.originalname} · ${(req.file.size/1024).toFixed(0)} KB`);
  ...
  try {
    ...
    res.json({ success: true, url, credits: req.creditsAfter });
  } catch (err) {
...
app.post('/api/upscale', requireAuth, upload.single('image'), requireCredits('upscale'), async (req, res) => {
  if (!req.file) return res.status(400).json({ success: false, error: 'No se recibió ninguna imagen.' });

  const scale = Math.min(8, Math.max(2, parseInt(req.body.scale) || 2));
  console.log(`[upscale] ${req.user.email} · ${req.file.originalname} · ${scale}×`);
